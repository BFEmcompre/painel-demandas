...
import { awardTaskCompletionPoints } from '../../lib/gamification';
...
type ChecklistItem = {
  ...
  text: string;
  completed: boolean;
  check_mode?: 'simple' | 'shift' | 'weekly' | 'biweekly' | 'monthly' | 'interval' | null;
  requires_morning_check?: boolean | null;
  morning_cutoff?: string | null;
  morning_completed_at?: string | null;
  requires_afternoon_check?: boolean | null;
  afternoon_cutoff?: string | null;
  afternoon_completed_at?: string | null;
  monthly_day?: number | null;
  period_key?: string | null;
  interval_hours?: number | null;
  interval_window_start?: string | null;
  interval_window_end?: string | null;
  interval_completions?: string[] | null;
};

const CHECK_MODE_GROUPS: Array<{ mode: NonNullable<ChecklistItem['check_mode']> | 'simple'; label: string }> = [
  { mode: 'interval', label: 'Contínuo' },
  { mode: 'simple', label: 'Diário' },
  { mode: 'shift', label: 'Manhã e Tarde' },
  { mode: 'weekly', label: 'Semanal' },
  { mode: 'biweekly', label: 'Quinzenal' },
...
export function TaskDetails() {
  ...
  const toggleChecklistShift = async (
    itemId: string,
    ...
    const item = checklist.find((current) => current.id === itemId);
    if (!item) return;

    const nextValue = !currentValue;
    const field = shift === 'morning' ? 'morning_completed_at' : 'afternoon_completed_at';
    const nowIso = nextValue ? new Date().toISOString() : null;
    const nextMorning = shift === 'morning' ? nowIso : item.morning_completed_at;
    const nextAfternoon = shift === 'afternoon' ? nowIso : item.afternoon_completed_at;
    const morningOk = !item.requires_morning_check || Boolean(nextMorning);
    const afternoonOk = !item.requires_afternoon_check || Boolean(nextAfternoon);
    const nextCompleted = morningOk && afternoonOk;

    setUpdatingChecklistId(itemId);

    setChecklist((currentItems) =>
      currentItems.map((current) =>
        current.id === itemId ? { ...current, [field]: nowIso, completed: nextCompleted } : current
      )
    );
    ...
    try {
      ...
      const { error: updateError } = await supabase
        .from('checklist_items')
        .update({ [field]: nowIso, completed: nextCompleted })
        .eq('id', itemId);
      ...
      const { error: logError } = await supabase
        ...
        .insert({
          ...
          user_id: authData.user.id,
          user_name: profile?.name || 'Usuário',
          action: `${nextValue ? 'marcou' : 'desmarcou'} (${shiftLabel})`,
        });
      ...
      setChecklist((currentItems) =>
        currentItems.map((current) => (current.id === itemId ? item : current))
      );
  ...
  function isMonthlyItemDue(item: ChecklistItem) {
    if (!item.monthly_day) return true;
    const todayDay = new Date().toLocaleString('pt-BR', { timeZone: 'America/Sao_Paulo', day: 'numeric' });
    return Number(todayDay) === item.monthly_day;
  }

  function todayWeekdayBR() {
    const todayBR = new Date().toLocaleDateString('sv-SE', { timeZone: 'America/Sao_Paulo' });
    return new Date(`${todayBR}T12:00:00`).getDay();
  }

  // Semanal fica ativo o expediente inteiro (não esmaece) — só mostramos um
  // aviso de "vencido" se chegou fim de semana e ninguém marcou ainda.
  function isWeeklyOverdue(item: ChecklistItem) {
    if (item.completed) return false;
    const weekday = todayWeekdayBR();
    return weekday === 0 || weekday === 6;
  }

  function biweeklyHalfLabel(item: ChecklistItem) {
    if (!item.period_key) return '';
    return item.period_key.endsWith('H1') ? '1ª metade do mês' : '2ª metade do mês';
  }

  function nextIntervalDueAt(item: ChecklistItem): Date | null {
    const hours = item.interval_hours || 2;
    const completions = item.interval_completions || [];
    if (completions.length === 0) return null;
    const last = new Date(completions[completions.length - 1]);
    return new Date(last.getTime() + hours * 60 * 60 * 1000);
  }

  function canMarkIntervalNow(item: ChecklistItem) {
    const due = nextIntervalDueAt(item);
    return !due || due.getTime() <= Date.now();
  }

  const markIntervalCheck = async (itemId: string) => {
    if (!task || updatingChecklistId === itemId) return;
    ...
    const nextCompletions = [...(item.interval_completions || []), nowIso];

    setUpdatingChecklistId(itemId);

    setChecklist((currentItems) =>
      currentItems.map((current) =>
        current.id === itemId ? { ...current, interval_completions: nextCompletions } : current
      )
    );
    ...
    try {
      ...
      const { error: updateError } = await supabase
        .from('checklist_items')
        .update({ interval_completions: nextCompletions })
        .eq('id', itemId);
      ...
      const { error: logError } = await supabase
        ...
        .insert({
          ...
          user_id: authData.user.id,
          user_name: profile?.name || 'Usuário',
          action: 'marcou (checagem contínua)',
        });
  ...
  const handleCompleteTask = async () => {
    if (isReadOnlyFixedDailyTask) {
      toast.error(
        'Essa tarefa fixa é de um dia anterior e está disponível apenas para consulta.'
      );
    ...
    const hasIncompleteChecklist = checklist.some((item) => {
      if (item.check_mode === 'interval') return false;
      if (item.check_mode === 'monthly' && !isMonthlyItemDue(item)) return false;
    ...
    if (authData.user) {
      const rewardResult = await awardTaskCompletionPoints({
        taskId: task.id,
        title: task.title,
      ...
      if (!rewardResult.skipped && !rewardResult.error && rewardResult.totalAwarded !== 0) {
        const sign = rewardResult.totalAwarded > 0 ? '+' : '';
        successMessage = `${successMessage} ${sign}${rewardResult.totalAwarded} pontos registrados.`;
      }
    }
  ...
  function updateEditChecklistItem(itemId: string, patch: Partial<ChecklistItem>) {
    setEditChecklist((current) =>
      current.map((item) =>
  ...
  async function handleUpdateTask() {
    ...
    const recurringTemplateId = task.is_recurring
      ? task.id
      : task.recurring_parent_id;
    ...
    if (recurringTemplateId) {
      const templateUpdateData: any = {
        ...
        priority: editPriority,
        requires_photo: editRequiresPhoto,
        recurring_deadline: editDeadline,
      };
      ...
      const { data: futureChildren } = await supabase
        .from('tasks')
        .select('id, date')
        .eq('recurring_parent_id', recurringTemplateId)
        .gte('date', task.date)
        .neq('status', 'completed');

      if (futureChildren && futureChildren.length > 0) {
        for (const childTask of futureChildren) {
          await supabase
            .from('tasks')
            .update({
              title: editTitle,
              ...
              priority: editPriority,
              requires_photo: editRequiresPhoto,
              deadline: `${childTask.date}T${editDeadline}`,
            })
            .eq('id', childTask.id);
        }
      }
    ...
    for (const item of editChecklist) {
      ...
      const mode = item.check_mode || 'simple';

      const itemUpdate: any = {
        text: item.text.trim(),
        check_mode: mode,
        requires_morning_check: mode === 'shift' ? Boolean(item.requires_morning_check) : false,
        morning_cutoff: mode === 'shift' && item.requires_morning_check ? item.morning_cutoff : null,
        requires_afternoon_check: mode === 'shift' ? Boolean(item.requires_afternoon_check) : false,
        afternoon_cutoff: mode === 'shift' && item.requires_afternoon_check ? item.afternoon_cutoff : null,
        monthly_day: mode === 'monthly' ? (item.monthly_day || 1) : null,
        interval_hours: mode === 'interval' ? (item.interval_hours || 2) : null,
        interval_window_start: mode === 'interval' ? (item.interval_window_start || '08:00') : null,
        interval_window_end: mode === 'interval' ? (item.interval_window_end || '18:00') : null,
        period_key:
          mode === 'weekly' || mode === 'biweekly'
            ? item.period_key || computePeriodKey(mode, new Date(`${task.date}T12:00:00`))
            : null,
      };
    ...
    <div className="max-w-3xl min-h-screen bg-white dark:bg-[#0B0B0B] text-gray-900 dark:text-white p-1">
      ...
      ">
        ...
        {task.status === 'overdue' &&
          isReadOnlyFixedDailyTask && (
            <p className="text-sm text-red-600 dark:text-red-400 mt-2">
              Tarefa fixa atrasada — disponível apenas para consulta.
            </p>
          )}
      </Card>
      ...
      {isAdmin && (
        <Card className="p-6 mb-6 bg-white dark:bg-[#121212] border border-gray-200 dark:border-[#1F1F1F]">
          ...
          {editingTask && task.status !== 'completed' && (
            <div className="flow-task-edit-scroll space-y-5">
              <div>
              ...
              <div className="space-y-3">
                ...
                {editChecklist.length === 0 ? (
                  ...
                  editChecklist.map((item) => {
                    const mode = item.check_mode || 'simple';
                    return (
                      <div key={item.id} className="space-y-2 rounded-2xl border border-gray-200 p-3 dark:border-[#2A2A2A]">
                        ...
                        <div className="flex flex-wrap items-center gap-2 pl-1">
                          <Label className="text-xs text-gray-600 dark:text-[#A1A1A1]">Frequência</Label>
                          <Select
                            value={mode}
                            onValueChange={(value) => updateEditChecklistItem(item.id, { check_mode: value as ChecklistItem['check_mode'] })}
                          >
                            <SelectTrigger className="h-8 w-48 bg-white text-xs dark:bg-[#181818] border-gray-300 dark:border-[#2A2A2A] text-gray-900 dark:text-white">
                              <SelectValue />
                            </SelectTrigger>
                            <SelectContent>
                        ...
                        {mode === 'shift' && (
                          <div className="flex flex-wrap items-center gap-4 pl-1 text-xs text-gray-600 dark:text-[#A1A1A1]">
                            <label className="flex items-center gap-1.5">
                              <Checkbox
                                checked={Boolean(item.requires_morning_check)}
                                onCheckedChange={(checked) => updateEditChecklistItem(item.id, { requires_morning_check: Boolean(checked) })}
                              />
                              Manhã, até
                              <input
                                type="time"
                                disabled={!item.requires_morning_check}
                                value={item.morning_cutoff || '12:00'}
                                onChange={(e) => updateEditChecklistItem(item.id, { morning_cutoff: e.target.value })}
                                className="h-7 w-24 rounded border px-2 text-xs bg-white border-gray-300 dark:bg-[#181818] dark:border-[#2A2A2A] text-gray-900 dark:text-white"
                              />
                            </label>

                            <label className="flex items-center gap-1.5">
                              <Checkbox
                                checked={Boolean(item.requires_afternoon_check)}
                                onCheckedChange={(checked) => updateEditChecklistItem(item.id, { requires_afternoon_check: Boolean(checked) })}
                              />
                              Tarde, até
                              <input
                                type="time"
                                disabled={!item.requires_afternoon_check}
                                value={item.afternoon_cutoff || '18:00'}
                                onChange={(e) => updateEditChecklistItem(item.id, { afternoon_cutoff: e.target.value })}
                                className="h-7 w-24 rounded border px-2 text-xs bg-white border-gray-300 dark:bg-[#181818] dark:border-[#2A2A2A] text-gray-900 dark:text-white"
                              />
                            </label>
                          </div>
                        ...
                        {mode === 'biweekly' && (
                          <p className="pl-1 text-xs text-gray-600 dark:text-[#A1A1A1]">
                            Precisa de uma marcação por metade do mês (dias 1–15 e 16–fim).
                          </p>
                        )}

                        {mode === 'monthly' && (
                          <div className="flex flex-wrap items-center gap-2 pl-1 text-xs text-gray-600 dark:text-[#A1A1A1]">
                            <span>Libera pra marcar no dia</span>
                            <input
                              type="number"
                              min={1}
                              max={31}
                              value={item.monthly_day || 1}
                              onChange={(e) => updateEditChecklistItem(item.id, { monthly_day: Math.min(31, Math.max(1, Number(e.target.value) || 1)) })}
                              className="h-7 w-16 rounded border px-2 text-xs bg-white border-gray-300 dark:bg-[#181818] dark:border-[#2A2A2A] text-gray-900 dark:text-white"
                            />
                        ...
                        {mode === 'interval' && (
                          <div className="flex flex-wrap items-center gap-2 pl-1 text-xs text-gray-600 dark:text-[#A1A1A1]">
                            ...
                            <input
                              ...
                              min={1}
                              max={12}
                              value={item.interval_hours || 2}
                              onChange={(e) => updateEditChecklistItem(item.id, { interval_hours: Math.max(1, Number(e.target.value) || 1) })}
                              className="h-7 w-16 rounded border px-2 text-xs bg-white border-gray-300 dark:bg-[#181818] dark:border-[#2A2A2A] text-gray-900 dark:text-white"
                            />
                            <span>hora(s), entre</span>
                            <input
                              type="time"
                              value={item.interval_window_start || '08:00'}
                              onChange={(e) => updateEditChecklistItem(item.id, { interval_window_start: e.target.value })}
                              className="h-7 w-24 rounded border px-2 text-xs bg-white border-gray-300 dark:bg-[#181818] dark:border-[#2A2A2A] text-gray-900 dark:text-white"
                            />
                            <span>e</span>
                            <input
                              type="time"
                              value={item.interval_window_end || '18:00'}
                              onChange={(e) => updateEditChecklistItem(item.id, { interval_window_end: e.target.value })}
                              className="h-7 w-24 rounded border px-2 text-xs bg-white border-gray-300 dark:bg-[#181818] dark:border-[#2A2A2A] text-gray-900 dark:text-white"
                            />
                          </div>
                        )}
              ...
              <div className="flow-task-edit-actions">
                <Button
                  type="button"
      ...
      ">
        ...
        {isReadOnlyFixedDailyTask && (
          <div className="
            ...
            dark:text-[#A1A1A1]
          ">
            Esta é uma tarefa fixa de um dia anterior.
            Ela está disponível apenas para consulta.
          </div>
        )}

        {CHECK_MODE_GROUPS.map((group) => {
          const groupItems = checklist.filter((c) => (c.check_mode || 'simple') === group.mode);
          if (groupItems.length === 0) return null;

          return (
            <div key={group.mode} className="mb-4">
              <p className="mb-1.5 text-[11px] font-black uppercase tracking-wider text-gray-500 dark:text-[#707070]">
                {group.label}
              </p>
              ...
              >
                {item.check_mode === 'shift' ? (
                  <div className="flex flex-1 flex-wrap items-center gap-x-4 gap-y-1">
                    ...
                    {item.requires_morning_check && (
                      <label className="flex items-center gap-1.5 text-xs text-gray-600 dark:text-[#A1A1A1]">
                        <Checkbox
                          checked={Boolean(item.morning_completed_at)}
                          onCheckedChange={() =>
                            toggleChecklistShift(item.id, 'morning', Boolean(item.morning_completed_at))
                          }
                          disabled={!canInteractWithTask || isUpdating}
                        />
                        Manhã{item.morning_cutoff ? ` (até ${item.morning_cutoff.slice(0, 5)})` : ''}
                      </label>
                    )}

                    {item.requires_afternoon_check && (
                      <label className="flex items-center gap-1.5 text-xs text-gray-600 dark:text-[#A1A1A1]">
                        <Checkbox
                          checked={Boolean(item.afternoon_completed_at)}
                          onCheckedChange={() =>
                            toggleChecklistShift(item.id, 'afternoon', Boolean(item.afternoon_completed_at))
                          }
                          disabled={!canInteractWithTask || isUpdating}
                        />
                        Tarde{item.afternoon_cutoff ? ` (até ${item.afternoon_cutoff.slice(0, 5)})` : ''}
                      </label>
                    )}
                  </div>
                ) : item.check_mode === 'weekly' ? (
                  (() => {
                    const overdue = isWeeklyOverdue(item);
                    return (
                      <div className="flex flex-1 items-center gap-2">
                        <Checkbox
                          ...
                          disabled={!canInteractWithTask || isUpdating}
                        />
                        <span className={item.completed ? 'line-through text-gray-500 dark:text-[#707070]' : 'text-gray-900 dark:text-white'}>
                          {item.text}
                        </span>
                        <span className={`text-xs ${overdue ? 'font-bold text-red-600 dark:text-red-400' : 'text-gray-500 dark:text-[#707070]'}`}>
                          {overdue ? '(vencido — não marcado até sexta)' : '(vale a semana toda, até sexta)'}
                        </span>
                      </div>
                    );
                  })()
                ) : item.check_mode === 'biweekly' ? (
                  <div className="flex flex-1 items-center gap-2">
                    <Checkbox
                      checked={item.completed}
                      onCheckedChange={() => toggleChecklist(item.id, item.completed)}
                    ...
                    <span className="text-xs text-gray-500 dark:text-[#707070]">
                      ({biweeklyHalfLabel(item)})
                    </span>
                  </div>
                ) : item.check_mode === 'monthly' ? (
                  (() => {
                    const due = isMonthlyItemDue(item);
                    return (
                      <div className={`flex flex-1 items-center gap-2 ${due ? '' : 'opacity-40'}`}>
                        <Checkbox
                          checked={item.completed}
                          onCheckedChange={() => toggleChecklist(item.id, item.completed)}
                          disabled={!due || !canInteractWithTask || isUpdating}
                        />
                        <span className={item.completed ? 'line-through text-gray-500 dark:text-[#707070]' : 'text-gray-900 dark:text-white'}>
                          {item.text}
                        </span>
                        <span className="text-xs text-gray-500 dark:text-[#707070]">
                          {due ? '(disponível hoje)' : `(disponível dia ${item.monthly_day})`}
                        </span>
                      </div>
                    );
                  })()
                ) : item.check_mode === 'interval' ? (
                  (() => {
                    const completions = item.interval_completions || [];
                    const canMarkNow = canMarkIntervalNow(item);
                    const due = nextIntervalDueAt(item);
                    return (
                      <div className="flex flex-1 flex-wrap items-center gap-3">
                        <span className="text-gray-900 dark:text-white">{item.text}</span>
                        <span className="text-xs text-gray-500 dark:text-[#707070]">
                          {completions.length}x hoje
                          {!canMarkNow && due ? ` · próxima às ${due.toLocaleTimeString('pt-BR', { hour: '2-digit', minute: '2-digit' })}` : ''}
                        </span>
                        <Button
                          type="button"
                          size="sm"
                          variant="outline"
                          onClick={() => markIntervalCheck(item.id)}
                          disabled={!canMarkNow || !canInteractWithTask || isUpdating}
                          className="h-7 bg-white text-xs dark:bg-[#181818] border-gray-300 dark:border-[#2A2A2A]"
                        >
                          Marcar checagem
                        </Button>
                      </div>
