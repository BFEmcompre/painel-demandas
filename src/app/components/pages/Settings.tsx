...
export function Settings() {
  ...
  async function handleAvatarUpload(file: File) {
    ...
    if (error) {
      ...
      toast.error('Não foi possível enviar a foto. Rode a migration V6 no Supabase.');
      return;
    }

    const { data } = supabase.storage.from('flow-avatars').getPublicUrl(path);
    ...
    <div className="max-w-4xl space-y-6 min-h-screen bg-white dark:bg-[#0B0B0B] text-gray-900 dark:text-white p-1">
      ...
      <div>
        ...
        <p className="text-gray-500 dark:text-[#A1A1A1] mt-1">
          Atualize seu perfil e as preferências da sua conta.
        </p>
      </div>
      ...
      ">
        ...
        <div className="space-y-4">
          ...
          <div className="flow-profile-photo-row">
            <div className="flow-profile-avatar-wrap">
              {avatarUrl ? (
                <img src={avatarUrl} alt="Foto de perfil" className="flow-profile-avatar" />
              ) : (
                <div className="flow-profile-avatar flow-profile-avatar-fallback">
                  <Camera className="h-7 w-7" />
                </div>
              )}
            </div>
            <div className="min-w-0 flex-1">
              <Label className="text-gray-900 dark:text-white">Foto de perfil</Label>
              <p className="mt-1 text-sm text-gray-500 dark:text-[#A1A1A1]">
                Essa foto aparece no ranking e no seu perfil dentro do Flow.
              </p>
            </div>
            <label className="flow-upload-button shrink-0">
