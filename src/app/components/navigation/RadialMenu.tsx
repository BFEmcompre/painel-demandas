...
export function RadialMenu({ open, onOpenChange }: RadialMenuProps) {
  ...
  const items = useMemo(
    () => (isManager ? managerItems : responsibleItems),
    [isManager],
  );
  ...
  return (
    <>
      <div
        className={`flow-radial-overlay ${open ? 'is-open' : ''}`}
        aria-hidden={!open}
        onMouseDown={(event) => {
      ...
      >
        <div className="flow-radial-stage" role="dialog" aria-modal="true" aria-label="Navegação do Flow">
          ...
          <div className="flow-radial-wheel">
            <div className="flow-radial-wheel-grid" />
            <div className="flow-radial-wheel-glow" />
            ...
            {items.map((item, index) => {
              ...
              return (
                <button
                  key={item.path}
                  type="button"
                  className={`flow-radial-item ${active ? 'is-active' : ''}`}
                  style={{ left: `${x}%`, top: `${y}%` }}
          ...
          <div className="flow-radial-help">
            <span><kbd>1–{items.length}</kbd> selecionar</span>
            <span><kbd>ESC</kbd> fechar</span>
            <span><kbd>Alt + Q</kbd> abrir</span>
          </div>
        </div>
      </div>
    </>
